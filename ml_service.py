...
@app.route('/ml/analogs', methods=['POST'])
def get_analogs():
    """Поиск аналогов квартиры"""
    try:
        data = request.json
        ...
        target_flat = data.get('target_flat')
        candidate_flats = data.get('candidate_flats', [])

        if not target_flat:
            return jsonify({'error': 'target_flat is required'}), 400

        if not candidate_flats:
            return jsonify({'error': 'candidate_flats is empty'}), 400

        # Предсказание цены для целевой квартиры
        input_data = preprocess_input(target_flat)
        ...
        for flat in candidate_flats:
            try:
                similarity = calculate_similarity_for_service(
                ...
                logger.error(f"Ошибка расчета аналога: {ex}")
...
if __name__ == '__main__':
    ...
    if load_models():
        ...
        logger.error("Не удалось загрузить модели. Сервер не запущен.")# ml_service.py
import os
import pickle
import joblib
import pandas as pd
...
db_pool = None
db_config = {
    'host': os.environ.get('DB_HOST', 'https://brusnika-grade.online'),
    'port': os.environ.get('DB_PORT', '5432'),
    'database': 'RealEstateDb',
    'user': 'postgres',
    'password': 'vfrcbv2005',
